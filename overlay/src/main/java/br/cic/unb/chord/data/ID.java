...
/**
 * Identifier for nodes and user-defined objects. New instances of this class are created either when a node joins the network, or by the local node
 * inserting a user-defined object.
 * 
 * Once created, an ID instance is unmodifiable.
 * 
 * IDs of same length can be compared as this class implements java.lang.Comparable. IDs of different length cannot be compared.
 */
public final class ID implements Comparable<ID>, Serializable
{
     ...
     * <code>br.cic.unb.chord.data.id.displayed.representation</code>. Possible values: <br/>
     * <br/>
     * 
    ...
    /**
     * The number of (highest) bytes of an id returned as String when {@link #toString()} is invoked. Is initialized with help of property
     * <code>br.cic.unb.chord.data.id.number.of.displayed.bytes</code>.
     */
    ...
    /**
     * Creates a new ID consisting of the given byte[] array. The ID is assumed to have (ID.length * 8) bits. It must have leading zeros if its value
     * has fewer digits than its maximum length.
     * 
     * @param id1
    ...
    /**
     * Returns length of this ID measured in bits. ID length is determined by the length of the stored byte[] array, i.e. leading zeros have to be
     * stored in the array.
     * 
    ...
    /**
     * Compare current ID with the given object. If either the object is not a ID or both IDs' lengths do not match, a ClassCastException is thrown.
    ...
    public final int compareTo(ID otherKey) throws ClassCastException
    {
        ...
        {
            throw new ClassCastException("Only ID objects with same length can be compared! This ID is " + this.id.length
                    + " bits long while the other ID is " + otherKey.getLength() + " bits long.");
        }
    ...
    /**
     * Checks if this ID is in the interval determined by the two given IDs. Neither of the boundary IDs is included in the interval. If both IDs
